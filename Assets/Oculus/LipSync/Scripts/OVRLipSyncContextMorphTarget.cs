...
public class OVRLipSyncContextMorphTarget : MonoBehaviour
{
    ...
    void Update ()
    {
        if ((lipsyncContext != null) && (skinnedMeshRenderers != null && skinnedMeshRenderers.Length > 0))
        {
            ...
            //{
                //if (frame.Visemes[i] > 0.01f)
                    //Debug.Log($"Viseme[{i}] = {frame.Visemes[i]}");
            //}
    ...
    void SetVisemeToMorphTarget(OVRLipSync.Frame frame)
    {
        for (int i = 0; i < visemeToBlendTargets.Length; i++)
        {
            ...
            {
                float weight = frame.Visemes[i] * 1.0f;
                foreach (var renderer in skinnedMeshRenderers)
                {
                    if (renderer != null)
                    {
                        renderer.SetBlendShapeWeight(visemeToBlendTargets[i], weight);
                    }
                }
            }
        }
    ...
    void SetLaughterToMorphTarget(OVRLipSync.Frame frame)
    {
        if (laughterBlendTarget != -1)
        {
            ...
            foreach (var renderer in skinnedMeshRenderers)
            {
                if (renderer != null && laughterBlendTarget >= 0 && laughterBlendTarget < renderer.sharedMesh.blendShapeCount)
                {
                    renderer.SetBlendShapeWeight(laughterBlendTarget, laughterScore);
                }
            }
        }
    }
